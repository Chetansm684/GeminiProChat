...
function useEventListener(...args) {
  ...
  const stopWatch = shared.watch(
    () => [shared.toValue(target), shared.toValue(options)],
    ([el, options2]) => {
      cleanup();
      if (!el)
        return;
...
function onClickOutside(target, handler, options = {}) {
  ...
  const shouldIgnore = (event) => {
    return ignore.some((target2) => {
      ...
      const el = shared.toValue(target2);
...
function onLongPress(target, handler, options) {
  var _a, _b;
  const elementRef = shared.toAccessor(target);
  let timeout;
  function clear() {
    if (timeout) {
...
function useAnimate(target, keyframes, options) {
  ...
  const [animate, setAnimate] = solidJs.createSignal(void 0);
  const store$1 = store.createMutable({
    startTime: null,
    currentTime: null,
    timeline: null,
  ...
  const pending = solidJs.createMemo(() => store$1.pending);
  const playState = solidJs.createMemo(() => store$1.playState);
  const replaceState = solidJs.createMemo(() => store$1.replaceState);
  const startTime = solidToVue.writableComputed({
    get() {
      return store$1.startTime;
    },
    set(value) {
      store$1.startTime = value;
      const animateVal = animate();
      if (animateVal)
        setAnimate(Object.assign(animateVal, { startTime: value }));
    }
  });
  const currentTime = solidToVue.writableComputed({
    get() {
      return store$1.currentTime;
    },
    set(value) {
      const animateVal = animate();
      if (animateVal) {
  ...
  const timeline = solidToVue.writableComputed({
    get() {
      return store$1.timeline;
    },
    set(value) {
      store$1.timeline = value;
      const animateVal = animate();
      if (animateVal)
        setAnimate(Object.assign(animateVal, { timeline: value }));
    }
  });
  const playbackRate = solidToVue.writableComputed({
    get() {
      return store$1.playbackRate;
    },
    set(value) {
      store$1.playbackRate = value;
      const animateVal = animate();
      if (animateVal)
        setAnimate(Object.assign(animateVal, { playbackRate: value }));
    }
  });
  const play = () => {
    const animateVal = animate();
  ...
  const pause = () => {
    var _a;
    try {
      (_a = animate()) == null ? void 0 : _a.pause();
      syncPause();
    } catch (e) {
      onError(e);
    }
  ...
  const reverse = () => {
    ...
    !animate() && update();
    try {
      (_a = animate()) == null ? void 0 : _a.reverse();
      syncResume();
    } catch (e) {
      syncPause();
      onError(e);
  ...
  const finish = () => {
    var _a;
    try {
      (_a = animate()) == null ? void 0 : _a.finish();
      syncPause();
    } catch (e) {
      onError(e);
    }
  ...
  const cancel = () => {
    var _a;
    try {
      (_a = animate()) == null ? void 0 : _a.cancel();
      syncPause();
    } catch (e) {
      onError(e);
    }
  ...
  solidJs.createEffect(
    solidJs.on(toAccessor(target), (el) => {
      el && update();
    })
  );
  solidJs.createEffect(
    solidJs.on(toAccessor(keyframes), (value) => {
      const animateVal = animate();
      !animateVal && update();
      if (!shared.toValue(target) && animateVal) {
        setAnimate(
          Object.assign(animateVal, {
            effect: new KeyframeEffect(shared.toValue(target), shared.toValue(value), animateOptions)
          })
        );
      }
    })
  ...
  shared.tryOnMount(() => {
    shared.nextTick(() => update(true));
  });
  shared.tryOnCleanup(cancel);
  function update(init) {
    const el = shared.toValue(target);
    if (!isSupported() || !el)
      return;
    const animateValue = el.animate(shared.toValue(keyframes), animateOptions);
    setAnimate(animateValue);
    if (commitStyles)
      animateValue.commitStyles();
    if (persist)
      animateValue.persist();
    if (_playbackRate !== 1)
      animateValue.playbackRate = _playbackRate;
    if (init && !immediate)
      animateValue.pause();
    else
      syncResume();
    onReady == null ? void 0 : onReady(animateValue);
  }
  ...
  const { resume: resumeRef, pause: pauseRef } = useRafFn(
    () => {
      ...
      if (!animateValue)
        return;
      store$1.pending = animateValue.pending;
      store$1.playState = animateValue.playState;
      store$1.replaceState = animateValue.replaceState;
      store$1.startTime = animateValue.startTime;
      store$1.currentTime = animateValue.currentTime;
      store$1.timeline = animateValue.timeline;
      store$1.playbackRate = animateValue.playbackRate;
    },
    { immediate: false }
  );
  function syncResume() {
...
function useAsyncState(promise, initialState, options) {
  ...
  function waitUntilIsLoaded() {
    return new Promise((resolve, reject) => {
      shared.until(isLoading, owner).toBe(false).then(() => resolve(shell)).catch(reject);
    });
  }
  return {
    ...shell,
...
function useMediaQuery(query, options = {}) {
  ...
  const update = () => {
    if (!isSupported())
      return;
    cleanup();
    mediaQuery = window2.matchMedia(shared.toAccessor(query)());
    setMatches(!!(mediaQuery == null ? void 0 : mediaQuery.matches));
    if (!mediaQuery)
      return;
...
function useBreakpoints(breakpoints, options = {}) {
  ...
  return Object.assign(shortcutMethods, {
    ...
    current() {
      ...
      return solidJs.createMemo(() => points.filter(([, v]) => v()).map(([k]) => k));
    }
  });
}
...
const WRITABLE_PROPERTIES = ["hash", "host", "hostname", "href", "pathname", "port", "protocol", "search"];
function useBrowserLocation({ window: window2 = defaultWindow } = {}) {
  const signals = Object.fromEntries(WRITABLE_PROPERTIES.map((key) => [key, solidJs.createSignal()]));
  for (const [key, sg] of shared.objectEntries(signals)) {
    solidJs.createEffect(
      solidJs.on(sg[0], (value) => {
  ...
  const buildState = (trigger) => {
    ...
    for (const key of WRITABLE_PROPERTIES)
      signals[key][1]((_a = window2 == null ? void 0 : window2.location) == null ? void 0 : _a[key]);
    return {
      trigger,
      state: state2,
      length,
  ...
  const state = store.createMutable(buildState("load"));
  if (window2) {
    useEventListener(window2, "popstate", () => Object.assign(state, buildState("popstate")), { passive: true });
    useEventListener(window2, "hashchange", () => Object.assign(state, buildState("hashchange")), { passive: true });
  }
  return state;
}
...
function useCloned(source, options = {}) {
  ...
  function sync() {
    setCloned(clone(shared.toValue(source)));
  }
...
function useStorage(key, defaults2, storage, options = {}) {
  ...
  const rawInit = shared.toValue(defaults2);
...
function useColorMode(options = {}) {
  ...
  const system = solidJs.createMemo(() => preferredDark() ? "dark" : "light");
  const [store2, setStore] = storageSignal ?? (storageKey == null ? solidToVue.toSignal(initialValue) : useStorage(storageKey, initialValue, storage, { window: window2, listenToStorageChanges }));
  const state = solidJs.createMemo(() => {
    const value = store2();
    return value === "auto" ? system() : value;
  });
  const updateHTMLAttrs = getSSRHandler("updateHTMLAttrs", (selector2, attribute2, value) => {
    const el = typeof selector2 === "string" ? window2 == null ? void 0 : window2.document.querySelector(selector2) : shared.toValue(selector2);
    if (!el)
      return;
    let style;
  ...
  function defaultOnChanged(mode2) {
    updateHTMLAttrs(selector, attribute, modes[mode2] ?? mode2);
  }
  ...
  const [mode, setMode] = solidToVue.writableComputed({
    get() {
      return emitAuto ? store2() : state();
    },
    set(v) {
      setStore(v);
    }
  });
  return { mode, setMode, store: store2, setStore, system, state };
}
function useConfirmDialog([revealed, setRevealed] = solidJs.createSignal(false)) {
  ...
  return {
    isRevealed: solidJs.createMemo(() => revealed()),
    reveal,
    confirm,
    cancel,
...
const toVec = (t) => (typeof t === "number" ? [t] : t) ?? [];
function executeTransition([source, setSource], from, to, options = {}) {
  const fromVal = shared.toValue(from);
  const toVal = shared.toValue(to);
  const v1 = toVec(fromVal);
  const v2 = toVec(toVal);
  const duration = shared.toValue(options.duration) ?? 1e3;
  const startedAt = Date.now();
  const endAt = Date.now() + duration;
  const trans = shared.toValue(options.transition) ?? shared.identity;
  const ease = typeof trans === "function" ? trans : createEasingFunction(trans);
  return new Promise((resolve) => {
    setSource(() => fromVal);
    const tick = () => {
      var _a;
      if ((_a = options.abort) == null ? void 0 : _a.call(options)) {
        resolve();
        return;
      }
      const now = Date.now();
      ...
      if (Array.isArray(source()))
        setSource(() => arr.map((_n, i) => lerp(v1[i] ?? 0, v2[i] ?? 0, alpha)));
...
function useCssTransition(source, options = {}) {
  ...
  const [output, setOutput] = solidJs.createSignal(sourceVal());
  solidJs.createEffect(
    solidJs.on(
      () => sourceVal(),
      async (to) => {
  ...
  solidJs.createEffect(
    solidJs.on(shared.toAccessor(options.disabled), (disabled) => {
      if (disabled) {
        currentId++;
        setOutput(sourceVal());
      }
  ...
  return solidJs.createMemo(() => shared.toValue(options.disabled) ? sourceVal() : output());
}
function useMutationObserver(target, callback, options = {}) {
  ...
  const stopWatch = shared.watch(shared.toAccessor(target), (el) => {
    cleanup();
...
function useCssVar(prop, target, options = {}) {
  ...
  const elAccessor = solidJs.createMemo(() => {
    var _a;
    return shared.toValue(target) ?? ((_a = window2 == null ? void 0 : window2.document) == null ? void 0 : _a.documentElement);
  });
  function updateCssVar() {
    var _a;
    const key = shared.toValue(prop);
    const el = shared.toValue(elAccessor);
    if (el && window2) {
  ...
  solidJs.createEffect(solidJs.on([elAccessor, shared.toAccessor(prop)], updateCssVar));
  solidJs.createEffect(
    solidJs.on(
      variable,
      (val) => {
        ...
        if ((_a = elAccessor()) == null ? void 0 : _a.style) {
          (_b = elAccessor()) == null ? void 0 : _b.style.setProperty(shared.toValue(prop), val);
        }
      },
      { defer: true }
    )
...
function useCycleList(list, options) {
  const [state, setState] = solidToVue.toSignal(getInitialValue());
  const listAccessor = shared.toAccessor(list);
  const index = solidJs.createMemo(() => {
    const targetList = listAccessor();
    let index2 = (options == null ? void 0 : options.getIndexOf) ? options.getIndexOf(state(), targetList) : targetList.indexOf(state());
    if (index2 < 0)
  ...
  function getInitialValue() {
    return (options == null ? void 0 : options.initialValue) ?? shared.toValue(list)[0];
  }
  solidJs.createEffect(
    solidJs.on(listAccessor, () => {
      set(index());
    })
  );
  return {
...
function useDraggable(target, options = {}) {
  ...
  const position = store.createMutable({ value: shared.toValue(options.initialValue) ?? { x: 0, y: 0 } });
  ...
  const handleEvent = (e) => {
    if (shared.toValue(options.preventDefault))
      e.preventDefault();
    if (shared.toValue(options.stopPropagation))
      e.stopPropagation();
  };
  const start = (e) => {
    var _a;
    if (!filterEvent(e))
      return;
    if (shared.toValue(options.exact) && e.target !== shared.toValue(target))
      return;
...
function useResizeObserver(target, callback, options = {}) {
  ...
  const targets = solidJs.createMemo(() => Array.isArray(target) ? target.map(shared.toValue) : [shared.toValue(target)]);
  const stopWatch = shared.watch(
    targets,
    (els) => {
      cleanup();
...
function useElementByPoint(options) {
  ...
  const controls = useRafFn(() => {
    setElement(() => (document2 == null ? void 0 : document2.elementFromPoint(shared.toValue(x), shared.toValue(y))) ?? null);
  });
  return {
    element,
    ...controls
...
function useElementSize(target, initialSize = { width: 0, height: 0 }, options = {}) {
  ...
  const isSVG = solidJs.createMemo(() => {
    var _a, _b;
    return (_b = (_a = shared.toValue(target)) == null ? void 0 : _a.namespaceURI) == null ? void 0 : _b.includes("svg");
  });
  ...
  useResizeObserver(
    ...
    ([entry]) => {
      ...
      if (window2 && isSVG()) {
        const $elem = shared.toValue(target);
        if ($elem) {
...
function useIntersectionObserver(target, callback, options = {}) {
  const { root, rootMargin = "0px", threshold = 0.1, window: window2 = defaultWindow, immediate = true } = options;
  const isSupported = useSupported(() => window2 && "IntersectionObserver" in window2);
  const targets = solidJs.createMemo(() => {
    const _target = shared.toValue(target);
    return (Array.isArray(_target) ? _target : [_target]).map(shared.toValue).filter(shared.notNullish);
  });
  let cleanup = shared.noop;
  const [isActive, setIsActive] = solidJs.createSignal(immediate);
  const stopWatch = isSupported() ? shared.watch(
    () => [targets(), shared.toValue(root), isActive],
    ([targets2, root2, isActive2]) => {
      cleanup();
      if (!isActive2)
        return;
      if (!targets2.length)
...
function useFetch(url, ...args) {
  ...
  const [error, setError] = solidJs.createSignal(null);
  const [data, setData] = solidJs.createSignal(initialData || null);
  ...
  const refetch = shared.toAccessor(options.refetch);
  solidJs.createEffect(
    solidJs.on(
      [refetch, shared.toAccessor(url)],
      ([refetch2]) => {
        refetch2 && execute();
      },
      { defer: true }
    ...
    return (payload, payloadType) => {
      if (!isFetching()) {
        ...
        if (solidToVue.isAccessor(config.payload)) {
          solidJs.createEffect(
            solidJs.on(
              [refetch, shared.toAccessor(config.payload)],
              ([refetch2]) => {
                refetch2 && execute();
              },
              { defer: true }
...
function useFileSystemAccess(options = {}) {
  ...
  async function open(_options = {}) {
    if (!isSupported())
      return;
    const [handle] = await window2.showOpenFilePicker({ ...shared.toValue(options), ..._options });
  ...
  solidJs.createEffect(solidJs.on(shared.toAccessor(dataType), updateData, { defer: true }));
  return {
    isSupported,
    data,
    setData,
...
function useFocus(target, options = {}) {
  ...
  const targetElement = shared.toAccessor(target);
...
function useFocusWithin(target, options = {}) {
  ...
  const targetElement = solidJs.createMemo(() => shared.toValue(target));
...
function useFullscreen(target, options = {}) {
  ...
  const targetAccessor = solidJs.createMemo(() => shared.toValue(target) ?? (document2 == null ? void 0 : document2.querySelector("html")));
  const [isFullscreen, setIsFullscreen] = solidJs.createSignal(false);
  const requestMethod = solidJs.createMemo(() => {
    return [
      "requestFullscreen",
      ...
      "msRequestFullscreen"
    ].find((m) => document2 && m in document2 || targetAccessor() && m in targetAccessor());
  });
  const exitMethod = solidJs.createMemo(() => {
    return [
      "exitFullscreen",
  ...
  const fullscreenEnabled = solidJs.createMemo(() => {
    return [
      "fullScreen",
      "webkitIsFullScreen",
  ...
  const isSupported = useSupported(
    () => targetAccessor() && document2 && requestMethod() !== void 0 && exitMethod() !== void 0 && fullscreenEnabled() !== void 0
  );
  const isCurrentElementFullScreen = () => {
    if (fullscreenElementMethod)
      return (document2 == null ? void 0 : document2[fullscreenElementMethod]) === targetAccessor();
    return false;
  };
  const isElementFullScreen = () => {
    const fullscreenEnabledValue = fullscreenEnabled();
    if (fullscreenEnabledValue) {
      if ((document2 == null ? void 0 : document2[fullscreenEnabledValue]) != null) {
        return document2[fullscreenEnabledValue];
      }
      const target2 = targetAccessor();
      if ((target2 == null ? void 0 : target2[fullscreenEnabledValue]) != null) {
        return Boolean(target2[fullscreenEnabledValue]);
      }
    }
    return false;
  };
  async function exit() {
    ...
    if (exitMethodValue) {
      if ((document2 == null ? void 0 : document2[exitMethodValue]) != null) {
        await document2[exitMethodValue]();
      } else {
        const target2 = targetAccessor();
        if ((target2 == null ? void 0 : target2[exitMethodValue]) != null)
          await target2[exitMethodValue]();
      }
    }
  ...
  async function enter() {
    ...
    if (requestMethodValue && (target2 == null ? void 0 : target2[requestMethodValue]) != null) {
      await target2[requestMethodValue]();
  ...
  useEventListener(() => shared.toValue(targetAccessor), eventHandlers, handlerCallback, false);
  if (autoExit)
...
const useImage = (options, asyncStateOptions = {}) => {
  const state = useAsyncState(() => loadImage(shared.toValue(options)), void 0, {
    resetOnExecute: true,
    ...asyncStateOptions
  });
  solidJs.createEffect(
...
function useScroll(element, options = {}) {
  ...
  function scrollTo(_x, _y) {
    ...
    (_a = _element instanceof Document ? document.body : _element) == null ? void 0 : _a.scrollTo({
      top: shared.toValue(_y) ?? y(),
      left: shared.toValue(_x) ?? x(),
...
function useInfiniteScroll(element, onLoadMore, options = {}) {
  ...
  const [promise, setPromise] = solidJs.createSignal();
  const isLoading = solidJs.createMemo(() => !!promise());
      ...
      if (!promise()) {
        setPromise(
          Promise.all([onLoadMore(state), new Promise((resolve) => setTimeout(resolve, interval))]).finally(() => {
            setPromise(null);
            solidToVue.nextTick(() => checkAndLoad());
          })
        );
      }
    }
  }
  solidJs.createEffect(solidJs.on(() => [state.arrivedState[direction], shared.toValue(element)], checkAndLoad));
  return {
    isLoading
  };
}
    ...
    get(target2, prop, rec) {
      ...
      if (!(prop in refs)) {
        if (/[+_-]/.test(prop)) {
          ...
          refs[prop] = solidJs.createMemo(() => keys2.every((key) => shared.toValue(proxy[key])));
        } else {
...
function useMediaControls(target, options = {}) {
  ...
  const targetAccessor = shared.toAccessor(target);
  solidJs.createEffect(
    solidJs.on(
  ...
  useEventListener(
    target,
    "timeupdate",
    () => ignoreCurrentTimeUpdates(() => setCurrentTime(() => shared.toValue(target).currentTime))
  );
  useEventListener(target, "durationchange", () => setDuration(() => shared.toValue(target).duration));
  useEventListener(target, "progress", () => setBuffered(() => timeRangeToArray(shared.toValue(target).buffered)));
  ...
  useEventListener(target, "ratechange", () => setRate(() => shared.toValue(target).playbackRate));
...
function useMousePressed(options = {}) {
  ...
  const target = solidJs.createMemo(() => shared.toValue(options.target) ?? window2);
...
function useNow(options = {}) {
  ...
  const [now, setNow] = solidJs.createSignal(/* @__PURE__ */ new Date());
...
function useObjectUrl(object) {
  ...
  solidJs.createEffect(
    solidJs.on(shared.toAccessor(object), (newObject) => {
      release();
      if (newObject) {
...
function useClamp(value, min, max) {
  if (solidToVue.isAccessor(value)) {
    return solidJs.createMemo(() => shared.clamp(shared.toValue(value), shared.toValue(min), shared.toValue(max)));
  }
  const [_value, _setValue] = solidToVue.toSignal(value);
  const getValue2 = solidJs.createMemo(() => {
    return shared.clamp(_value(), shared.toValue(min), shared.toValue(max));
  });
  const setValue = (value2) => {
    const res = shared.clamp(solidToVue.getSetterValue(value2, _value()), shared.toValue(min), shared.toValue(max));
    _setValue(res);
    return res;
  };
...
function useOffsetPagination(options) {
  ...
  const [currentPageSize, setCurrentPageSize] = useClamp(
    setPageSize === void 0 || typeof pageSize === "number" ? pageSize : [pageSize, setPageSize],
    1,
    Infinity
  );
  const pageCount = solidJs.createMemo(() => Math.max(1, Math.ceil(shared.toValue(total) / shared.toValue(currentPageSize))));
  const [currentPage, setCurrentPage] = useClamp(
    setPage === void 0 || typeof page === "number" ? page : [page, setPage],
    1,
    pageCount
  );
...
function useParentElement(element = useActiveElement()) {
  const [parentElement, setParentElement] = solidJs.createSignal();
  const update = () => {
    const el = shared.toValue(element);
    if (el)
  ...
  solidJs.createEffect(solidJs.on(shared.toAccessor(element), update));
...
function usePointerLock(target, options = {}) {
  ...
  function lock(e, options2) {
    ...
    targetElement = e instanceof Event ? shared.toValue(target) ?? triggerElementValue : shared.toValue(e);
    if (!targetElement)
...
function usePointerSwipe(target, options = {}) {
  const targetRef = shared.toAccessor(target);
...
function usePrevious(value, initialValue) {
  ...
  solidJs.createEffect((v) => {
    setPrevious(() => v);
    return shared.toAccessor(value)();
  }, initialValue);
  return previous;
}
...
function useScriptTag(src, onLoaded = shared.noop, options = {}) {
  ...
  const loadScript = (waitForScriptLoad) => new Promise((resolve, reject) => {
    ...
    let el = document2.querySelector(`script[src="${shared.toValue(src)}"]`);
    if (!el) {
  ...
  const unload = () => {
    ...
    if (scriptTag())
      setScriptTag(null);
    const el = document2.querySelector(`script[src="${shared.toValue(src)}"]`);
    if (el)
...
function useScrollLock(element, initialState = false) {
  ...
  let initialOverflow;
  solidJs.createEffect(
    solidJs.on(shared.toAccessor(element), (el) => {
      if (el) {
        const ele = el;
...
function useSpeechRecognition(options = {}) {
  ...
  const lang = shared.toAccessor(options.lang ?? "en-US");
...
function useSpeechSynthesis(text, options = {}) {
  ...
  const spokenText = shared.toAccessor(text || "");
  const lang = shared.toAccessor(options.lang ?? "en-US");
  const [error, setError] = solidJs.createSignal(void 0);
  ...
  if (isSupported()) {
    ...
    if (options.voice) {
      solidJs.createEffect(
        solidJs.on(
          shared.toAccessor(options.voice),
          () => {
            synth.cancel();
          },
          { defer: true }
...
function useStepper(steps, initialStep) {
  const stepsRef = shared.toAccessor(steps);
...
function useTextareaAutoSize(options) {
  ...
  const [textareaScrollHeight, setTextareaScrollHeight] = solidJs.createSignal(1);
  function triggerResize() {
    ...
    setTextareaScrollHeight((_a = textareaRef()) == null ? void 0 : _a.scrollHeight);
    if (options == null ? void 0 : options.styleTarget)
      shared.toValue(options.styleTarget).style.height = `${textareaScrollHeight()}px`;
    else
...
function useTimeAgo(time, options = {}) {
  ...
  const timeAgo = solidJs.createMemo(() => formatTimeAgo(new Date(shared.toValue(time)), options, shared.toValue(now())));
  if (exposeControls) {
    return {
      timeAgo,
      ...controls
...
function useUrlSearchParams(mode = "history", options = {}) {
  ...
  if (!window2)
    return solidJs.createSignal(initialValue);
  const [state, setState] = solidJs.createSignal({});
...
function useUserMedia(options = {}) {
  ...
  const [constraints, setConstraints] = solidToVue.toSignal(options.constraints);
  const { navigator = defaultNavigator } = options;
  const isSupported = useSupported(() => {
    var _a;
  ...
  function getDeviceOptions(type) {
    ...
    switch (type) {
      case "video": {
        if (constraintsValue)
          return constraintsValue.video ?? false;
        break;
      }
      case "audio": {
        if (constraintsValue)
          return constraintsValue.audio ?? false;
        break;
      }
    }
  }
...
function useVibrate(options) {
  ...
  const patternAccessor = shared.toAccessor(pattern);
...
function useWatchForSizes(size, list, calculateRange) {
  shared.watch(
    [size.width, size.height, shared.toAccessor(list)],
    () => {
      calculateRange();
    },
    { defer: true }
...
function useWebSocket(url, options = {}) {
  ...
  const urlAccessor = shared.toAccessor(url);
  let heartbeatPause;
  let heartbeatResume;
...
const depsParser$1 = depsParser;
const createWorkerBlobUrl = (fn, deps) => {
  const blobCode = `${depsParser$1(deps)}; onmessage=(${jobRunner$1})(${fn})`;
  const blob = new Blob([blobCode], { type: "text/javascript" });
  const url = URL.createObjectURL(blob);
  return url;
};
const createWorkerBlobUrl$1 = createWorkerBlobUrl;
const useWebWorkerFn = (fn, options = {}) => {
  ...
  const generateWorker = () => {
    const blobUrl = createWorkerBlobUrl$1(fn, dependencies);
