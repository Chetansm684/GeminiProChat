import { noop, toValue, isClient, watch, tryOnCleanup, isIOS, toAccessor as toAccessor$1, isObject, objectOmit, tryOnMount, nextTick, promiseTimeout, until, increaseWithUnit, objectEntries, useTimeoutFn, pausableWatch, createEventHook, identity, timestamp, pausableFilter, watchIgnorable, debounceFilter, createFilterWrapper, bypassFilter, createSingletonPromise, notNullish, containsProp, hasOwn, throttleFilter, useDebounceFn, useThrottleFn, useIntervalFn, clamp, syncSignal, toAccessors, objectPick, watchWithFilter } from "@solidjs-use/shared";
export * from "@solidjs-use/shared";
import { createEffect, createSignal, getOwner, onMount, createMemo, on, runWithOwner, onCleanup } from "solid-js";
import { createMutable } from "solid-js/store";
import { writableComputed, isAccessor as isAccessor$1, toSignal, nextTick as nextTick$1, isSignal, set, del, toAccessor as toAccessor$2, getSetterValue, reactive } from "@solidjs-use/shared/solid-to-vue";
...
function onLongPress(target, handler, options) {
  var _a, _b;
  const elementRef = toAccessor$1(target);
  let timeout;
  function clear() {
    if (timeout) {
...
function useMediaQuery(query, options = {}) {
  ...
  const update = () => {
    if (!isSupported())
      return;
    cleanup();
    mediaQuery = window2.matchMedia(toAccessor$1(query)());
...
function useStorage(key, defaults2, storage, options = {}) {
  ...
  if (!storage)
    return isAccessor$1(defaults2) ? data : [data, setData];
...
function useCssTransition(source, options = {}) {
  ...
  createEffect(
    on(toAccessor$1(options.disabled), (disabled) => {
      if (disabled) {
        currentId++;
        setOutput(sourceVal());
      }
...
function useMutationObserver(target, callback, options = {}) {
  ...
  const stopWatch = watch(toAccessor$1(target), (el) => {
    cleanup();
...
function useCssVar(prop, target, options = {}) {
  ...
  createEffect(on([elAccessor, toAccessor$1(prop)], updateCssVar));
  createEffect(
    on(
      variable,
      (val) => {
...
function useCycleList(list, options) {
  ...
  const listAccessor = toAccessor$1(list);
...
function useFetch(url, ...args) {
  ...
  const refetch = toAccessor$1(options.refetch);
  createEffect(
    on(
      [refetch, toAccessor$1(url)],
      ([refetch2]) => {
        refetch2 && execute();
      },
      { defer: true }
  ...
  function setMethod(method) {
    return (payload, payloadType) => {
      if (!isFetching()) {
        ...
        if (isAccessor$1(config.payload)) {
          createEffect(
            on(
              [refetch, toAccessor$1(config.payload)],
              ([refetch2]) => {
                refetch2 && execute();
              },
              { defer: true }
...
function useFileSystemAccess(options = {}) {
  ...
  createEffect(on(toAccessor$1(dataType), updateData, { defer: true }));
  return {
    isSupported,
    data,
    setData,
...
function useFocus(target, options = {}) {
  ...
  const targetElement = toAccessor$1(target);
...
function useMediaControls(target, options = {}) {
  ...
  const targetAccessor = toAccessor$1(target);
  createEffect(
    on(
...
function useMouseInElement(target, options = {}) {
  ...
  const targetRef = toAccessor$2(target ?? (window2 == null ? void 0 : window2.document.body));
...
function useNow(options = {}) {
  ...
  const [now, setNow] = createSignal(/* @__PURE__ */ new Date());
...
function useObjectUrl(object) {
  ...
  createEffect(
    on(toAccessor$1(object), (newObject) => {
      release();
      if (newObject) {
...
function useParentElement(element = useActiveElement()) {
  ...
  tryOnMount(update);
  createEffect(on(toAccessor$1(element), update));
...
function usePointerSwipe(target, options = {}) {
  const targetRef = toAccessor$1(target);
...
function useSpeechRecognition(options = {}) {
  ...
  const lang = toAccessor$1(options.lang ?? "en-US");
...
function useSpeechSynthesis(text, options = {}) {
  ...
  const spokenText = toAccessor$1(text || "");
  const lang = toAccessor$1(options.lang ?? "en-US");
  const [error, setError] = createSignal(void 0);
...
function useStepper(steps, initialStep) {
  const stepsRef = toAccessor$1(steps);
...
function useTitle(newTitle = null, options = {}) {
  ...
  const isReadonly = !!(newTitle && isAccessor$1(newTitle));
  function format(t) {
...
function useVibrate(options) {
  ...
  const patternAccessor = toAccessor$1(pattern);
...
function useWatchForSizes(size, list, calculateRange) {
  watch(
    [size.width, size.height, toAccessor$1(list)],
    () => {
      calculateRange();
    },
    { defer: true }
...
function useWebSocket(url, options = {}) {
  ...
  const urlAccessor = toAccessor$1(url);
  let heartbeatPause;
  let heartbeatResume;
