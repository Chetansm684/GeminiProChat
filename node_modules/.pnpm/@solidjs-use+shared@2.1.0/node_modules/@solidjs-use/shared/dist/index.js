...
const resolveAccessor = toAccessor;
...
function useArrayDifference(...args) {
  ...
  return solidJs.createMemo(() => toValue(list).filter((x) => toValue(values).findIndex((y) => compareFn(x, y)) === -1));
}
function useArrayEvery(list, fn) {
  return solidJs.createMemo(() => toValue(list).every((element, index, array) => fn(toValue(element), index, array)));
}
...
function useArrayFindIndex(list, fn) {
  return solidJs.createMemo(() => toValue(list).findIndex((element, index, array) => fn(toValue(element), index, array)));
}
...
function useArraySome(list, fn) {
  return solidJs.createMemo(() => toValue(list).some((element, index, array) => fn(toValue(element), index, array)));
}
function uniq(array) {
...
function useDateFormat(date, formatStr = "HH:mm:ss", options = {}) {
  return solidJs.createMemo(() => formatDate(normalizeDate(toValue(date)), toValue(formatStr), options));
}
...
function useSorted(...args) {
  ...
  if (!dirty)
    return solidJs.createMemo(() => sortFn([...toValue(source)], compareFn));
  solidJs.createEffect(() => {
...
function useToString(value) {
  return solidJs.createMemo(() => `${toValue(value)}`);
}
function useToggle(initialValue = false, options = {}) {
  const { truthyValue = true, falsyValue = false } = options;
  const [_value, setValue] = solidToVue.toSignal(initialValue);
