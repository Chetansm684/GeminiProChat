...
// node_modules/.pnpm/@solidjs-use+solid-to-vue@2.1.0/node_modules/@solidjs-use/solid-to-vue/dist/index.mjs
function writableComputed(options) {
  return [options.get, options.set];
}
var reactive = createMutable;
...
// node_modules/.pnpm/@solidjs-use+shared@2.1.0/node_modules/@solidjs-use/shared/dist/index.mjs
function toValue(r) {
  return typeof r === "function" ? r() : r;
}
var unAccessor = toValue;
...
function increaseWithUnit(target, delta) {
  ...
  if (typeof target === "number")
    return target + delta;
  const value = ((_a = /^-?[0-9]+\.?[0-9]*/.exec(target)) == null ? void 0 : _a[0]) ?? "";
...
function omitMutable(obj, ...keys2) {
...
function pickMutable(obj, ...keys2) {
  const res = createMutable({});
  const validKeys = createMemo(() => {
    const flatKeys = keys2.flat();
    const allKeys = Object.keys(obj);
...
function toAccessor2(r) {
  return isAccessor(r) ? r : () => r;
}
var resolveAccessor = toAccessor2;
...
var defaultComparator = (value, othVal) => value === othVal;
...
function useArrayFilter(list, fn) {
  return createMemo(
    () => toValue(list).map((i) => toValue(i)).filter(fn)
  );
}
...
function useArrayIncludes(...args) {
  ...
  return createMemo(
    () => toValue(list).slice(formIndex).some((element, index, array) => comparator(toValue(element), toValue(value), index, toValue(array)))
  );
}
function useArrayJoin(list, separator) {
  return createMemo(
    () => toValue(list).map((i) => toValue(i)).join(toValue(separator))
  );
}
function useArrayMap(list, fn) {
  return createMemo(
    () => toValue(list).map((i) => toValue(i)).map(fn)
  );
}
...
function useAsyncQueue(tasks, options = {}) {
  const { interrupt = true, onError = noop2, onFinished = noop2 } = options;
  const promiseState = {
    pending: "pending",
    rejected: "rejected",
    fulfilled: "fulfilled"
  ...
  tasks.reduce((prev, curr) => {
    return prev.then((prevRes) => {
      var _a;
      if (((_a = result[activeIndex()]) == null ? void 0 : _a.state) === promiseState.rejected && interrupt) {
        onFinished();
        return;
      }
...
function useCounter(initialValue = 0, options = {}) {
  ...
  const set2 = (val) => setCount(Math.max(min, Math.min(max, val)));
  const reset = (val = initialValue) => {
    initialValue = val;
    return set2(val);
  };
  return { count, inc, dec, set: set2, reset };
}
...
var formatDate = (date, formatStr, options = {}) => {
  ...
  return formatStr.replace(REGEX_FORMAT, (match, $1) => {
    var _a;
    return $1 || ((_a = matches[match]) == null ? void 0 : _a.call(matches)) || match;
  });
};
...
function useSorted(...args) {
  ...
  if (!dirty)
    return createMemo(() => sortFn([...toValue(source)], compareFn));
  createEffect(() => {
...
function useTimeout(interval = 1e3, options = {}) {
  ...
  const controls = useTimeoutFn(callback ?? noop2, interval, options);
...
// node_modules/.pnpm/solidjs-use@2.1.0/node_modules/solidjs-use/dist/index.mjs
...
function createAsyncMemo(evaluationCallback, initialState, optionsOrSetter) {
  ...
  const { setEvaluating = void 0, onError = noop2 } = options;
...
function useEventListener(...args) {
  ...
  const stopWatch = watch(
    () => [toValue(target), toValue(options)],
    ([el, options2]) => {
      cleanup();
      if (!el)
        return;
...
function onLongPress(target, handler, options) {
  var _a, _b;
  const elementRef = toAccessor2(target);
  let timeout;
  function clear() {
    if (timeout) {
...
function isAccessor2(val) {
  return typeof val === "function";
}
function toAccessor3(r) {
  return isAccessor2(r) ? r : () => r;
}
function useMounted() {
...
function useAnimate(target, keyframes, options) {
  ...
  const [animate, setAnimate] = createSignal(void 0);
  ...
  createEffect(
    on(toAccessor3(target), (el) => {
      el && update();
    })
  );
  createEffect(
    on(toAccessor3(keyframes), (value) => {
      const animateVal = animate();
      !animateVal && update();
...
function useAsyncState(promise, initialState, options) {
  ...
  function waitUntilIsLoaded() {
    return new Promise((resolve, reject) => {
      until(isLoading, owner).toBe(false).then(() => resolve(shell)).catch(reject);
    });
  }
  return {
    ...shell,
...
function useMediaQuery(query, options = {}) {
  ...
  const update = () => {
    if (!isSupported())
      return;
    cleanup();
    mediaQuery = window2.matchMedia(toAccessor2(query)());
...
var WRITABLE_PROPERTIES = ["hash", "host", "hostname", "href", "pathname", "port", "protocol", "search"];
function useBrowserLocation({ window: window2 = defaultWindow } = {}) {
  const signals = Object.fromEntries(WRITABLE_PROPERTIES.map((key) => [key, createSignal()]));
...
var globalKey = "__solidjs-use_ssr_handlers__";
...
function useStorage(key, defaults2, storage, options = {}) {
  ...
  const { pause: pauseWatch, resume: resumeWatch } = watchPausable(data, () => write(data()), {
    eventFilter,
    defer: false
  });
...
function useColorMode(options = {}) {
  ...
  const [store, setStore] = storageSignal ?? (storageKey == null ? toSignal(initialValue) : useStorage(storageKey, initialValue, storage, { window: window2, listenToStorageChanges }));
  const state = createMemo(() => {
    const value = store();
    return value === "auto" ? system() : value;
  });
  const updateHTMLAttrs = getSSRHandler("updateHTMLAttrs", (selector2, attribute2, value) => {
    const el = typeof selector2 === "string" ? window2 == null ? void 0 : window2.document.querySelector(selector2) : toValue(selector2);
    if (!el)
      return;
    let style;
...
function useConfirmDialog([revealed, setRevealed] = createSignal(false)) {
...
var lerp = (a, b, alpha) => a + alpha * (b - a);
var toVec = (t) => (typeof t === "number" ? [t] : t) ?? [];
function executeTransition([source, setSource], from, to, options = {}) {
  const fromVal = toValue(from);
  const toVal = toValue(to);
  const v1 = toVec(fromVal);
  const v2 = toVec(toVal);
  const duration = toValue(options.duration) ?? 1e3;
  const startedAt = Date.now();
  const endAt = Date.now() + duration;
  const trans = toValue(options.transition) ?? identity;
...
function useCssTransition(source, options = {}) {
  ...
  createEffect(
    on(toAccessor2(options.disabled), (disabled) => {
      if (disabled) {
        currentId++;
        setOutput(sourceVal());
      }
...
function useMutationObserver(target, callback, options = {}) {
  ...
  const stopWatch = watch(toAccessor2(target), (el) => {
    cleanup();
...
function useCssVar(prop, target, options = {}) {
  ...
  const elAccessor = createMemo(() => {
    var _a;
    return toValue(target) ?? ((_a = window2 == null ? void 0 : window2.document) == null ? void 0 : _a.documentElement);
  });
  ...
  createEffect(on([elAccessor, toAccessor2(prop)], updateCssVar));
  createEffect(
    on(
      variable,
      (val) => {
        ...
        if ((_a = elAccessor()) == null ? void 0 : _a.style) {
          (_b = elAccessor()) == null ? void 0 : _b.style.setProperty(toValue(prop), val);
        }
      },
      { defer: true }
    )
...
function useCycleList(list, options) {
  ...
  const listAccessor = toAccessor2(list);
  ...
  function getInitialValue() {
    return (options == null ? void 0 : options.initialValue) ?? toValue(list)[0];
  }
  createEffect(
    on(listAccessor, () => {
      set2(index());
...
var fnBypass = (v) => v;
var fnSetSource = (setSource, value) => setSource(value);
...
function useDeviceMotion(options = {}) {
  ...
  const [interval, setInterval2] = createSignal(0);
...
function useDraggable(target, options = {}) {
  ...
  const position = createMutable({ value: toValue(options.initialValue) ?? { x: 0, y: 0 } });
  ...
  const handleEvent = (e) => {
    if (toValue(options.preventDefault))
      e.preventDefault();
    if (toValue(options.stopPropagation))
      e.stopPropagation();
  };
  const start = (e) => {
    var _a;
    if (!filterEvent(e))
      return;
    if (toValue(options.exact) && e.target !== toValue(target))
      return;
...
function useElementByPoint(options) {
  ...
  const controls = useRafFn(() => {
    setElement(() => (document2 == null ? void 0 : document2.elementFromPoint(toValue(x), toValue(y))) ?? null);
  });
  return {
    element,
    ...controls
...
function useElementSize(target, initialSize = { width: 0, height: 0 }, options = {}) {
  ...
  const isSVG = createMemo(() => {
    var _a, _b;
    return (_b = (_a = toValue(target)) == null ? void 0 : _a.namespaceURI) == null ? void 0 : _b.includes("svg");
  });
...
function useFetch(url, ...args) {
  ...
  const [error, setError] = createSignal(null);
  const [data, setData] = createSignal(initialData || null);
  ...
  const refetch = toAccessor2(options.refetch);
  createEffect(
    on(
      [refetch, toAccessor2(url)],
      ([refetch2]) => {
        refetch2 && execute();
      },
      { defer: true }
    ...
    return (payload, payloadType) => {
      if (!isFetching()) {
        ...
        if (isAccessor(config.payload)) {
          createEffect(
            on(
              [refetch, toAccessor2(config.payload)],
              ([refetch2]) => {
                refetch2 && execute();
              },
              { defer: true }
...
function useFileSystemAccess(options = {}) {
  ...
  createEffect(on(toAccessor2(dataType), updateData, { defer: true }));
  return {
    isSupported,
    data,
    setData,
...
function useFocus(target, options = {}) {
  ...
  const targetElement = toAccessor2(target);
...
function useFullscreen(target, options = {}) {
  ...
  const targetAccessor = createMemo(() => toValue(target) ?? (document2 == null ? void 0 : document2.querySelector("html")));
  ...
  const fullscreenEnabled = createMemo(() => {
    return [
      "fullScreen",
      "webkitIsFullScreen",
...
function useMagicKeys(options = {}) {
  const { target = defaultWindow, aliasMap = DefaultMagicKeysAliasMap, passive = true, onEventFired = noop2 } = options;
    ...
    get(target2, prop, rec) {
      ...
      if (!(prop in refs)) {
        if (/[+_-]/.test(prop)) {
          ...
          refs[prop] = createMemo(() => keys2.every((key) => toValue(proxy[key])));
        } else {
...
function useMediaControls(target, options = {}) {
  ...
  const targetAccessor = toAccessor2(target);
  createEffect(
    on(
...
function useMousePressed(options = {}) {
  ...
  const target = createMemo(() => toValue(options.target) ?? window2);
...
function useNow(options = {}) {
  ...
  const [now2, setNow] = createSignal(/* @__PURE__ */ new Date());
...
function useObjectUrl(object) {
  ...
  createEffect(
    on(toAccessor2(object), (newObject) => {
      release();
      if (newObject) {
...
function useClamp(value, min, max) {
  ...
  const setValue = (value2) => {
    const res = clamp(getSetterValue(value2, _value()), toValue(min), toValue(max));
    _setValue(res);
    return res;
  };
...
function useOffsetPagination(options) {
  const {
    ...
    setPageSize,
    onPageChange = noop2,
    onPageSizeChange = noop2,
    onPageCountChange = noop2
  } = options;
...
function useParentElement(element = useActiveElement()) {
  ...
  tryOnMount(update);
  createEffect(on(toAccessor2(element), update));
...
function usePointerSwipe(target, options = {}) {
  const targetRef = toAccessor2(target);
...
function useScriptTag(src, onLoaded = noop2, options = {}) {
  const {
    immediate = true,
    manual = false,
...
function useSpeechRecognition(options = {}) {
  ...
  const lang = toAccessor2(options.lang ?? "en-US");
...
function useSpeechSynthesis(text, options = {}) {
  ...
  const spokenText = toAccessor2(text || "");
  const lang = toAccessor2(options.lang ?? "en-US");
  const [error, setError] = createSignal(void 0);
...
function useStepper(steps, initialStep) {
  const stepsRef = toAccessor2(steps);
...
function checkPassiveEventSupport(document2) {
  ...
  document2.addEventListener("x", noop2, optionsBlock);
  document2.removeEventListener("x", noop2);
...
function useTextareaAutoSize(options) {
  ...
  function triggerResize() {
    ...
    if (options == null ? void 0 : options.styleTarget)
      toValue(options.styleTarget).style.height = `${textareaScrollHeight()}px`;
    else
...
function useTimeAgo(time, options = {}) {
  ...
  const timeAgo = createMemo(() => formatTimeAgo(new Date(toValue(time)), options, toValue(now2())));
  if (exposeControls) {
    return {
      timeAgo,
      ...controls
...
function useVibrate(options) {
  ...
  const patternAccessor = toAccessor2(pattern);
...
function useWatchForSizes(size, list, calculateRange) {
  watch(
    [size.width, size.height, toAccessor2(list)],
    () => {
      calculateRange();
    },
    { defer: true }
...
function useWebSocket(url, options = {}) {
  ...
  const urlAccessor = toAccessor2(url);
  let heartbeatPause;
  let heartbeatResume;
...
var createWorkerBlobUrl$1 = createWorkerBlobUrl;
...
//# sourceMappingURL=solidjs-use.js.map
